...
class ChatClient {
    ...
    // ── Inject voice conversation into activity bar on return from siri.html
    handleVoiceReturn() {
        const voiceTranscript = sessionStorage.getItem('voiceTranscript');
        const voiceResponse   = sessionStorage.getItem('voiceResponse');
        if (!voiceTranscript) return;

        setTimeout(() => {
            this.addMessageToUI(voiceTranscript, 'user');
            if (voiceResponse) this.addMessageToUI(voiceResponse, 'assistant');
        }, 200);

        sessionStorage.removeItem('voiceTranscript');
        sessionStorage.removeItem('voiceResponse');
    }
    ...
    clearSidebarHistory() {
        if (confirm("Clear all chat history?") && this.historyItems)
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    chatInstance.handleVoiceReturn();
});